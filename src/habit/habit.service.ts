...
export class HabitService {
  ...
  async handleStreakReset() {
    ...
    // Find all active habits with a streak, and get their most recent log.
    ...
    // Filter down to habits that were actually missed yesterday.
    const habitsToReset = activeHabits.filter((habit) => {
      if (habit.logs.length === 0) {
        // This should not happen if streak > 0, but it's a safeguard.
        return true;
      }
      const lastLogDate = habit.logs[0].date;
      // If the last log was from before yesterday, the streak is broken.
  ...
  async payToUpdate(
    ...
    return this.prisma.$transaction(async (tx) => {
      const habit = await this.findOne(id, userId); // Authorization check
      const user = await tx.user.findUnique({ where: { id: userId } });
      if (!user) throw new NotFoundException('User not found.');
  ...
  async logHabit(habitId: string, logHabitDto: LogHabitDto, userId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // A negative log no longer breaks the streak, the cron job does.
      // We just track the counters.
