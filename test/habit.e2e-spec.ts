...
describe('HabitController (e2e)', () => {
  ...
  let testUserId: string; // Variable to hold the user's ID
  ...
  beforeAll(async () => {
    ...
    // --- THIS IS THE FIX ---
    // 1. Register the user and immediately get their ID
    const registerResponse = await request(app.getHttpServer())
      .post('/auth/register')
      .send(testUser);
    testUserId = registerResponse.body.id; // Assign the ID here
    ...
    // 3. Now that testUserId has a value, update the user's gold
  ...
  afterAll(async () => {
    // Use the stored testUserId for cleanup
    if (testUserId) {
  ...
  describe('Habit Lifecycle and Logic', () => {
    ...
    it('/habits/:id/pay-to-delete (DELETE) - should successfully delete the habit by paying the cost', async () => {
      ...
      const cost = 50; // Cost for EASY difficulty
